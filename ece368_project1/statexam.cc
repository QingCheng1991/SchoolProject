...
int test1( )
{
    ...
    // Returns 38 if everything goes okay; otherwise returns 0.
    ...
    if (!close(s.sum( ), sum)) return 0;
    if (!close(s.mean( ), sum/10000)) return 0;
...
int main( )
{
    ...
    cerr << "TEST 1:" << endl;
    cerr << "Testing next, reset, length, sum, and mean (38 points).\n";
    result = test1( );
    value += result;
    ...
    cerr << "grade will be " << value << " points out of 80.\n";
